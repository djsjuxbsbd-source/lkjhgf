...
WELCOME_SERVICES_TEXT = (
    "🌿 Вітаю вас у бухгалтерському боті!\n\nЯ — ваш бухгалтер та помічник у питаннях ФОП.\nТут ви знайдете зрозумілі консультації, автоматичний супровід, реєстрацію/закриття ФОП та податкову допомогу.\n\nОберіть потрібну категорію нижче 👇"
)
...
FOP_REGISTER_PAY_TEXT = (
    ...
    "❤️ ОБОВ'ЯЗКОВО: після здійснення оплати надішліть, будь ласка, чек або скрін на @your_telegram_tag або в розділ (Поставити питання, у меню)"
)
...
DECRET_CONTACTS_TEXT = (
    ...
    "▪ Контактний телефон\n\n"
    "Після отримання цих даних підготуємо необхідні документи та розпочнемо про��едуру.\n\n"
...
def webhook():
    ...
    if "callback_query" in update:
        ...
        if data == "support_admin":
            # Поведінка аналогічна кнопці "Поставити питання"
    ...
    if user_id in decret_request and decret_request[user_id].get("stage") == "await_contact":
        ...
        if any(k in msg for k in ("photo", "document", "video", "audio", "voice")):
            ...
            send_message(ADMIN_ID, note, parse_mode="HTML", reply_markup=admin_reply_markup(user_id))
        send_message(user_id, "Дякуємо! Ваші дані отримано, розпочинаємо підготовку документів. Якщо ��отрібно щось ще — звертайтесь.", reply_markup=main_menu_markup())
        decret_request.pop(user_id, None)
        return "ok", 200
