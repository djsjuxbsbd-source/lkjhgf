...
# ======= Inline markup для декрет ФОП =======
def decret_inline():
    return {
        "inline_keyboard": [
            [{"text": "Хочу оформить", "callback_data": "decret_apply"}],
...
def decret_pay_inline():
    return {
        "inline_keyboard": [
            [{"text": "Оплатить / реквизиты", "callback_data": "decret_pay"}],
...
FOP_CLOSE_TEXT = (
    "Я допоможу офіційно припинити підприємницьку діяльність швидко, бе�� черг і зайвих клопотів.\n"
...
PRRO_REGISTER_TEXT = (
    "Надаю комплексну допомогу у реєстрації програмного реєстратора розрахункових операцій (ПРРО) відповідно до закона."
    "Що входить у послугу:\n"
...
# ======= Декрет ФОП (новые тексты, как по описанию) =======
DECRET_SERVICE_TEXT = (
    "Допомагаю правильно оформити та отримати декретні виплати відповідно до законодавства України.\n\n"
    "Що входить у послугу:\n"
    "⚪Консультація щодо прав на декретні виплати (батькам, майбутнім мамам, опікунам)\n"
    "⚪Підготовка та оформлення необхідних документів для отримання допомоги\n"
...
def webhook():
    ...
    if "callback_query" in update:
        ...
        if data in ("decret",):  # на случай, если кто-то ручками дернет
        ...
        if data.startswith("close_") and int(from_id) == ADMIN_ID:
            ...
            send_message(user_id, "⛔️ Чат завершён администратором. Ви вернулись у головне меню.", reply_markup=main_menu_markup())
    ...
    if text == "Завершить чат" and cid in active_chats:
        ...
        send_message(cid, "⛔️ Чат завершён. Ви вернулись у головне меню.", reply_markup=main_menu_markup())
    ...
    if cid == ADMIN_ID:
        ...
        if any(k in msg for k in ("photo", "document", "video", "audio", "voice")):
            ...
            send_message(target, "⛔️ Чат завершён администратором. Ви вернулись у головне меню.", reply_markup=main_menu_markup())
            send_message(ADMIN_ID, "Чат завершён.", reply_markup=main_menu_markup())
        elif text:
    ...
    if user_id in reports_request and reports_request[user_id].get("stage") == "await_contact":
        ...
        if req_type == "submit":
            ...
            send_message(ADMIN_ID, note, parse_mode="HTML", reply_markup=admin_reply_markup(user_id))
            send_message(user_id, "Дякую! Ваші дані отримано, звітність буде підготовлена найближчим часом. Якщо потрібно щось ще — звертайтесь.", reply_markup=main_menu_markup())
            reports_request.pop(user_id, None)
            return "ok", 200
            ...
            send_message(ADMIN_ID, note, parse_mode="HTML", reply_markup=admin_reply_markup(user_id))
            send_message(user_id, "Дякую! Перевірка буде виконана і вся інформація надана у відповідь. Якщо потрібно щось ще — звертайтесь.", reply_markup=main_menu_markup())
            reports_request.pop(user_id, None)
            return "ok", 200

    # === ОБРАБОТКА КОНТАКТОВ ДЛЯ ДЕКРЕТУ (новое) ===
    if user_id in decret_request and decret_request[user_id].get("stage") == "await_contact":
        ...
        if any(k in msg for k in ("photo", "document", "video", "audio", "voice")):
            ...
            send_message(ADMIN_ID, note, parse_mode="HTML", reply_markup=admin_reply_markup(user_id))
        send_message(user_id, "Дякую! Ваші дані отримано, розпочинаємо підготовку документів. Якщо потрібно щось ще — звертайтесь.", reply_markup=main_menu_markup())
        decret_request.pop(user_id, None)
        return "ok", 200
